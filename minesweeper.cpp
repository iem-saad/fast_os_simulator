...
void placemines(int mines[][2],char realboard[][max_side])
{
	...
	//Initializing Mark array with false
...
{
cout << "The Location of mines are: " << endl;
...
void play()
{
	...
	int x,y;
	int mines[max_mine][2]; //stores (x,y) coordinates of all mines
...
void choosedifficulty()
{
	...
	do
	{
		...
		else if(choice == 4)
		{
			...
			cin.get();
   		system("./dealloc_resource MINESWEEPER");
			exit(0);
		}
...
int main()
{
  ...
  system("clear");
  system(" chmod +x ./welcome_minesweeper.sh");
  system("./welcome_minesweeper.sh");
